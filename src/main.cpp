...
enum GameState { MENU, PLAYING, GOAL, WIN };
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (state == PLAYING || state == GOAL) {
            ...
            // ---------- DETECCIÓN DE GOL ----------
            if (state == PLAYING && ballMoving && (goalScored || hitKeeper || out)) {
                ...
                if (goalScored && !hitKeeper) {
                    ...
                    goalText.setString("¡ATAJÓ EL PORTERO!");
                }
